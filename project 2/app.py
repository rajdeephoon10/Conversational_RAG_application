...
# yha se h working wla code 5 questions+history
## RAG Q&A Conversation With PDF Including Chat History
# import streamlit as st
# from langchain.chains import create_history_aware_retriever, create_retrieval_chain
# from langchain.chains.combine_documents import create_stuff_documents_chain
...
#     # prepare container for generated questions (so it persists across reruns)
...
#         vectorstore = Chroma.from_documents(documents=splits, embedding=embeddings)
#         retriever = vectorstore.as_retriever()
#         # Auto-Generate Top 5 Questions
#         # Combine text (use original documents content for better context)
#         combined_text = "\n\n".join([d.page_content for d in documents])
#         safe_text = combined_text[:15000]  # trim to avoid token limits; increase if you prefer

#         # 1) Summarize briefly (optional, helps question generation)
#         summary_prompt = f"""
...
#             summary_text = getattr(summary_res, "content", str(summary_res))
#         except Exception:
#             summary_text = safe_text[:4000]  # fallback to using raw text

#         # 2) Ask the model to return EXACTLY a JSON list of 5 questions
#         question_prompt = f"""
...
#             q_text = getattr(q_res, "content", str(q_res))
...
#                     parsed_clean = [str(x).strip() for x in parsed][:5]
#                     # if fewer than 5, fallback to further parsing
#                     if len(parsed_clean) == 5:
#                         st.session_state.auto_questions = parsed_clean
#                     else:
#                         # continue to fallback parsing below
#                         raise ValueError("Parsed list not length 5")
#                 else:
#                     raise ValueError("Parsed JSON not list")
#             except Exception:
#                 # fallback: attempt to extract the first JSON-like bracketed section
#                 try:
#                     start = q_text.find('[')
#                     end = q_text.rfind(']') + 1
...
#                         parsed_clean = [str(x).strip() for x in parsed2][:5]
#                         st.session_state.auto_questions = parsed_clean
#                     else:
#                         # last fallback: split by newlines and take non-empty lines
#                         lines = [ln.strip() for ln in q_text.splitlines() if ln.strip()]
#                         # remove any bullet markers and quotes
#                         cleaned = []
#                         for ln in lines:
#                             ln2 = ln.lstrip('-•0123456789. ').strip().strip('"').strip("'")
#                             if ln2:
#                                 cleaned.append(ln2)
#                         st.session_state.auto_questions = cleaned[:5] if cleaned else ["(No questions generated)"]
#                 except Exception:
#                     st.session_state.auto_questions = ["(No questions generated)"]
#         except Exception:
#             st.session_state.auto_questions = ["(Question generation failed)"]
#         # Display the generated questions as a simple list 
#         st.subheader("🔎 Suggested Questions From Your PDF")
#         if isinstance(st.session_state.auto_questions, list) and st.session_state.auto_questions:
#             for idx, q in enumerate(st.session_state.auto_questions, start=1):
#                 st.write(f"**Q{idx}:** {q}")
#         else:
#             st.write("No suggested questions available.")
...
#         # Prefilled input not used here; display as usual
#         user_input = st.text_input("Your question:")
...
## RAG Q&A Conversation With PDF Including Chat History (updated)
# import streamlit as st
# from langchain.chains import create_history_aware_retriever, create_retrieval_chain
# from langchain.chains.combine_documents import create_stuff_documents_chain
# from langchain_chroma import Chroma
...
# ## Helper to robustly extract text from various LLM wrapper return types
# def llm_text(res):
#     try:
#         # prefer .content
...
#         # some wrappers have .generations
...
#     # prepare container for generated questions (so it persists across reruns)
#     if 'auto_questions' not in st.session_state:
#         # dictionary mapping filename -> list of 5 questions
#         st.session_state.auto_questions = {}

#     # prepared field to hold a question (populated when clicking Ask)
...
#     if uploaded_files:
#         documents = []
#         file_docs_map = {}  # filename -> documents (pages) for that file
#         # Use unique temp files per upload and preserve source metadata
#         with st.spinner("Processing uploaded PDF(s) — extracting text and building embeddings..."):
#             try:
#                 for uploaded_file in uploaded_files:
#                     # write each upload to a unique temp file
...
#                 # Split and create embeddings for the documents (global vectorstore from all splits)
#                 text_splitter = RecursiveCharacterTextSplitter(chunk_size=5000, chunk_overlap=500)
#                 splits = text_splitter.split_documents(documents)
#                 vectorstore = Chroma.from_documents(documents=splits, embedding=embeddings)
#                 retriever = vectorstore.as_retriever()
...
#                 # -----------------------------
#                 # NEW: Generate 5 questions PER UPLOADED FILE
#                 # -----------------------------
#                 # reset dictionary for this run
#                 st.session_state.auto_questions = {}

#                 for fname, docs_for_file in file_docs_map.items():
#                     combined_text = "\n\n".join([d.page_content for d in docs_for_file])
#                     safe_text = combined_text[:15000]  # trim to keep prompt size reasonable

#                     # 1) Summarize briefly (helps generate focused questions)
#                     summary_prompt = f"""
# You are an expert at summarizing documents. Summarize the following text into 5-7 very short bullet points (one sentence each). Keep it concise.
...
#                     except Exception:
#                         summary_text = safe_text[:4000]

#                     # 2) Ask the model to return EXACTLY a JSON list of 5 questions for THIS file
#                     question_prompt = f"""
# You are a helpful assistant that generates exam-style or study questions.
# Based on the summary below, generate EXACTLY 5 high-quality, diverse, and meaningful questions someone should ask to understand this document.
# Return ONLY a valid JSON array of 5 strings and nothing else. Example:
...
#                     try:
#                         q_res = llm.invoke(question_prompt)
#                         q_text = llm_text(q_res)

#                         parsed_questions = None
#                         try:
#                             parsed_questions = json.loads(q_text)
#                         except Exception:
#                             # attempt to extract bracketed JSON if model added extra text
#                             start = q_text.find('[')
#                             end = q_text.rfind(']') + 1
#                             if start != -1 and end != -1:
#                                 try:
#                                     parsed_questions = json.loads(q_text[start:end])
#                                 except Exception:
#                                     parsed_questions = None
...
#                             lines = [ln.strip().lstrip('-•0123456789. ').strip('"').strip("'") for ln in q_text.splitlines() if ln.strip()]
#                             cleaned = [ln for ln in lines if ln][:5]
#                             if not cleaned:
#                                 cleaned = ["(No questions generated)"] * 5
#                             elif len(cleaned) < 5:
#                                 cleaned += ["(Not generated)"] * (5 - len(cleaned))
#                             st.session_state.auto_questions[fname] = cleaned

#                     except Exception:
#                         st.session_state.auto_questions[fname] = ["(Question generation failed)"] * 5
...
#         # -----------------------------
#         # Show the generated questions grouped by PDF file
#         # -----------------------------
...
#                     col1.write(f"**Q{i}:** {q}")
#                     # Clicking this Ask button fills the text input key 'user_question'
...
#         # -----------------------------
#         # History-aware retriever and QA chain (keeps your original flow)
#         # -----------------------------
#         contextualize_q_system_prompt = (
#             "Given a chat history and the latest user question"
#             "which might reference context in the chat history, "
...
#                 )
#                 st.write(st.session_state.store)
#                 st.write("Assistant:", response.get('answer') if isinstance(response, dict) else str(response))
#                 st.write("Chat History:", session_history.messages)
#             except Exception:
...
#                     loader = PyPDFLoader(tmp_path)
#                     docs = loader.load()
#                     # add filename + page metadata for traceability
...
#                 # --- IMPORTANT: build retriever with top_k to allow cross-document retrieval ---
#                 retriever = vectorstore.as_retriever(search_kwargs={"k": int(top_k)})

#                 # -----------------------------
#                 # Generate 5 questions PER UPLOADED FILE (unchanged)
#                 # -----------------------------
#                 # reset dictionary for this run
#                 st.session_state.auto_questions = {}
...
#         # -----------------------------
#         # History-aware retriever and QA chain (with cross-doc prompting)
#         # -----------------------------
#         contextualize_q_system_prompt = (
#             "Given a chat history and the latest user question "
#             "which might reference context in the chat history, "
...
#         ## Answer question
#         # --- Build system prompt depending on whether sentence→source mapping is requested ---
#         if show_sentence_source:
#             # Instruct the model to append a source tag to each sentence in the answer.
#             # Use format: (source: filename.pdf p.X)
#             system_prompt = (
#                 "You are an assistant for question-answering tasks. "
...
#                 "If a sentence is supported by multiple sources, list them separated by semicolons inside the parentheses, e.g. (source: file_A.pdf p.2; file_B.pdf p.14). "
...
#                 "You will be given several retrieved context chunks from possibly multiple documents (each chunk includes a source). "
#                 "Synthesize information across the retrieved chunks to answer the user's question. "
#                 "If your answer uses information from multiple documents, you may cite the sources in a final 'Sources:' line using the filename and page number where possible, e.g. [file_A.pdf p.2, file_B.pdf p.14]. "
...
#         def get_session_history(session: str) -> BaseChatMessageHistory:
#             # use the provided session key (safer than closing over session_id)
...
#         if user_input:
#             session_history = get_session_history(session_id)
#             # Retrieve top-k documents for display/verification (same retriever used by chain)
#             try:
#                 retrieved_docs = retriever.get_relevant_documents(user_input)
#             except Exception:
#                 retrieved_docs = []

#             # Show retrieved evidence so user can verify cross-document sources
...
#             else:
#                 st.info("Retrieved evidence display is hidden (toggle in sidebar).")

#             # Now call the conversational RAG chain to synthesize an answer across retrieved chunks
#             try:
#                 response = conversational_rag_chain.invoke(
#                     {"input": user_input},
#                     config={
...
#                 # If sentence->source mapping is enabled, attempt to parse the assistant text for tagged sentences
#                 if show_sentence_source:
#                     st.markdown("#### Sentence → Source mapping (parsed)")
#                     # Regex to find segments followed by (source: ...)
...
#                     else:
#                         st.write("Could not parse sentence→source tags from the assistant response. The assistant might not have followed the exact tagging format. Showing raw assistant text above.")

#                 # Show unique source filenames used by retriever (quick summary) — controlled by toggle
#                 if show_sources_ui:
#                     sources_set = []
...
#                 else:
#                     st.info("Sources display is hidden (toggle in sidebar).")
...
#                 # -----------------------------
#                 # Generate 5 questions PER UPLOADED FILE
#                 # -----------------------------
#                 # reset dictionary for this run
#                 st.session_state.auto_questions = {}
...
# """
#                     # -------------------------
#                     # Robust question-generation block with debug & fallbacks
#                     # -------------------------
#                     try:
#                         # Summarize
#                         summary_res = llm.invoke(summary_prompt)
...
#                         # If summary fails, show error and use raw text fallback
#                         st.error(f"[DEBUG] Summary generation failed for {fname}: {e}")
#                         st.text(traceback.format_exc())
#                         summary_text = safe_text[:4000]

#                     # Build question prompt and call LLM
#                     question_prompt = f"""
# You are a helpful assistant that generates exam-style or study questions.
# Based on the summary below, generate EXACTLY 5 high-quality, diverse, and meaningful questions someone should ask to understand this document.
# Return ONLY a valid JSON array of 5 strings and nothing else. Example:
...
#                         st.write(q_text[:1000])  # show first 1000 chars of LLM output for debugging
...
#                             parsed_questions = json.loads(q_text)
#                         except Exception:
#                             # 2) try to extract first bracketed JSON substring
#                             start = q_text.find('[')
#                             end = q_text.rfind(']') + 1
#                             if start != -1 and end != -1:
#                                 snippet = q_text[start:end]
...
#                         # 3) fallback: newline / bullet split
#                         if not isinstance(parsed_questions, list):
#                             lines = [ln.strip() for ln in q_text.splitlines() if ln.strip()]
#                             cleaned_lines = []
#                             for ln in lines:
#                                 # drop numbering/bullets and quotes
#                                 ln2 = ln.lstrip('-•0123456789. ').strip().strip('"').strip("'")
#                                 if ln2:
...
#                             st.session_state.auto_questions[fname] = cleaned
#                         else:
#                             # If LLM returned nothing usable, fallback to a local heuristic generator
#                             st.warning(f"[DEBUG] Could not parse questions from LLM for {fname}. Falling back to heuristic generator.")
#                             # Heuristic fallback: simple keyword-based generation (customize as needed)
...
#                     except Exception as e:
#                         # If the LLM call itself fails (network, auth, or model error), show trace and fallback
...
# # --------------------- SAFE RERUN HELPER ---------------------
# def safe_rerun():
#     """Attempt rerun safely across all Streamlit versions."""
...
#     # Fallback: toggle a dummy query param to force rerun
#     try:
#         key = "_rerun_toggle"
#         st.session_state[key] = 0 if st.session_state.get(key, 0) else 1
#         if hasattr(st, "experimental_set_query_params"):
#             st.experimental_set_query_params(_rerun=st.session_state[key])
#         return
#     except Exception:
#         pass

#     # Final fallback: stop app (user manually refreshes)
#     try:
#         st.stop()
#     except Exception:
#         return
...
# streamlit rerun problem solver
# --------------------- SAFE RERUN HELPER (updated to use st.query_params) ---------------------
def safe_rerun():
